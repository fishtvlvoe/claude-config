...
| L069 | **派 Kimi CLI 寫碼前 MUST 防護 SDD 檔案**。Kimi CLI 沒有排除目錄的 flag，`-w` 設工作目錄 = 它能動的全部範圍。防護 SOP：(1) 派工前 `git add openspec/ .claude/ && git commit -m "wip: pre-kimi checkpoint"` 保護 SDD 檔案；(2) `-w` 只指向程式碼目錄（如 `-w src/` 或 `-w apps/`），不指向專案根目錄；(3) prompt 結尾加明文禁令：「只修改程式碼檔案，禁止動 openspec/、.claude/、docs/ 目錄下的任何檔案」；(4) 派工後 `git diff --stat` 檢查有無意外改動，有則 `git checkout -- openspec/ .claude/`。**`--add-dir` 是「擴展」scope 不是「限制」**，不能拿來當白名單用。正確呼叫範例：`kimi --print -w src/ -p "重構 X 模組... 只修改 src/ 下的檔案，禁止動 openspec/、.claude/、docs/"` | Kimi CLI 派工 |

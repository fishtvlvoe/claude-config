...
| L059 | **Spectra propose 寫完所有 artifacts 後，MUST 自動跑 `spectra analyze` 並修復四個維度（Coverage/Consistency/Ambiguity/Gaps）的所有 Critical 和 Warning**。修完後再跑一次確認 0 Critical + 0 Warning 才進 validate。Suggestion 等級：掃到涉及資料轉換/排序/計算的 scenario 缺 concrete example → 補上；純 UI 導航的 scenario → 可跳過但要在回報時說明「22 個 Suggestion 中 N 個已補、M 個屬 UI 導航流程已跳過」。禁止：(1) 只修 Warning 不看 Suggestion 就說「完成」；(2) 不跑 analyze 就直接 validate；(3) analyze 有 Critical/Warning 還直接 park。Fish 不應該需要手動叫跑一致性檢查。 | Spectra propose 完成後 |
